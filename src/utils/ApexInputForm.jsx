import React from "react";

export default function ApexInputForm({ setPlatform, platform }) {
  return (
    <div class="flex items-start ">
      <button
        onClick={() => setPlatform("origin")}
        type="button"
        className={`group flex items-center rounded-l-lg justify-center h-12 w-12 transition ${
          platform === "origin"
            ? "bg-violet-400 text-gray-300"
            : "bg-gray-100 dark:bg-dark-700"
        }`}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className={`h-5 w-5 ${
            platform === "origin" ? "text-gray-100" : "text-gray-400"
          }}  group-hover:text-gray-500 dark:group-hover:text-gray-300`}
          viewBox="0 0 24 24"
        >
          <path
            d="M12.588 3.11c1.189.071 2.352.384 3.417.919 1.031.514 1.95 1.225 2.706 2.094.751.865 1.322 1.853 1.715 2.963.391 1.109.548 2.278.464 3.502-.033.636-.135 1.252-.306 1.848-.167.588-.393 1.159-.674 1.703-.439.849-.929 1.652-1.47 2.412-.538.759-1.125 1.465-1.762 2.118-.638.653-1.313 1.254-2.032 1.802-.719.544-1.471 1.038-2.254 1.479l-.037.026c-.033.018-.071.026-.109.023-.063-.015-.118-.048-.159-.097-.041-.05-.063-.111-.062-.173 0-.029.004-.059.012-.085.008-.023.021-.044.037-.062.277-.393.506-.806.686-1.235.181-.434.303-.885.368-1.359 0-.032-.015-.064-.038-.085-.021-.025-.053-.038-.085-.038-.264.032-.528.053-.795.062-.266.009-.532-.003-.796-.037-1.189-.071-2.353-.385-3.418-.918-1.031-.515-1.949-1.226-2.705-2.095-.754-.87-1.336-1.875-1.715-2.963-.394-1.123-.552-2.314-.465-3.502.033-.636.135-1.252.306-1.848.171-.598.396-1.155.675-1.68.439-.864.931-1.676 1.469-2.436.539-.757 1.125-1.464 1.761-2.118.639-.652 1.314-1.252 2.033-1.8.72-.546 1.47-1.039 2.253-1.479l.038-.025c.033-.02.07-.027.109-.025.065.016.119.051.158.098.043.051.062.106.062.174.001.027-.003.057-.012.084-.007.023-.02.043-.036.061-.273.386-.505.801-.687 1.237-.181.433-.3.885-.366 1.358 0 .033.012.063.036.086.022.024.054.037.085.037.262-.033.527-.053.795-.061.272-.009.536.003.798.035zm-.807 12.367c.922.079 1.838-.231 2.521-.855.72-.639 1.109-1.438 1.176-2.4.078-.928-.232-1.846-.856-2.535-.601-.708-1.472-1.131-2.4-1.162-.927-.078-1.845.232-2.534.855-.709.602-1.132 1.473-1.164 2.4-.078.926.228 1.842.846 2.535.628.725 1.432 1.115 2.411 1.162z"
            fill="currentColor"
          ></path>
          <title>origin</title>
        </svg>
      </button>
      <button
        onClick={() => setPlatform("psn")}
        type="button"
        className={`group flex items-center justify-center h-12 w-12 first:rounded-l last:rounded-r transition ${
          platform === "psn"
            ? "bg-violet-400 text-gray-300"
            : "bg-gray-100 dark:bg-dark-700"
        }`}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className={`h-5 w-5 ${
            platform === "psn" ? "text-gray-100" : "text-gray-400"
          }}  group-hover:text-gray-500 dark:group-hover:text-gray-300`}
          viewBox="0 0 24 24"
        >
          <path
            d="M8.985 2.596v17.548l3.915 1.261V6.688c0-.69.304-1.151.794-.991.636.181.76.814.76 1.505v5.876c2.441 1.193 4.362-.002 4.362-3.153 0-3.237-1.126-4.675-4.438-5.827-1.307-.448-3.728-1.186-5.391-1.502h-.002zm4.656 16.242l6.296-2.275c.715-.258.826-.625.246-.818-.586-.192-1.637-.139-2.357.123l-4.205 1.499v-2.385l.24-.085s1.201-.42 2.913-.615c1.696-.18 3.785.029 5.437.661 1.848.601 2.041 1.472 1.576 2.072s-1.622 1.036-1.622 1.036l-8.544 3.107v-2.297l.02-.023zM1.808 18.6c-1.9-.545-2.214-1.668-1.352-2.321.801-.585 2.159-1.051 2.159-1.051l5.616-2.013v2.313L4.206 17c-.705.271-.825.632-.239.826.586.195 1.637.15 2.343-.12L8.248 17v2.074c-.121.029-.256.044-.391.073-1.938.331-3.995.196-6.037-.479l-.012-.068z"
            fill="currentColor"
          ></path>
          <title>playstation</title>
        </svg>
      </button>
      <button
        onClick={() => setPlatform("xbox")}
        type="button"
        className={`group flex items-center justify-center h-12 w-12 rounded-r-lg transition ${
          platform === "xbox"
            ? "bg-violet-400 text-gray-300"
            : "bg-gray-100 dark:bg-dark-700"
        }`}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className={`h-5 w-5 ${
            platform === "xbox" ? "text-gray-100" : "text-gray-400"
          }}  group-hover:text-gray-500 dark:group-hover:text-gray-300`}
          viewBox="0 0 24 24"
        >
          <path
            d="M 12 0 C 10.08 0 7.68 0.6 5.88 1.68 C 5.76 1.8 5.4 2.04 5.16 2.28 C 6.84 0.36 11.88 4.32 12 4.44 C 12.12 4.32 17.16 0.36 18.84 2.28 C 18.6 2.04 18.24 1.8 18.12 1.68 C 16.32 0.6 14.28 0 12 0 Z M 4.44 3.12 C 4.08 3.12 3.6 3.48 3.24 3.84 C 1.2 6 0 8.88 0 12 C 0 15.12 1.2 17.88 3.12 20.04 C 2.52 17.04 3.12 14.76 4.8 12 C 6.6 9.12 9.6 6 9.6 6 C 7.08 3.6 5.28 3.24 4.8 3.12 C 4.68 3.12 4.56 3.12 4.44 3.12 Z M 3.12 20.04 C 5.28 22.44 8.52 24 12 24 C 15.48 24 18.72 22.44 20.88 20.04 L 20.88 20.04 C 22.8 17.88 24 15.12 24 12 C 24 8.88 22.8 6 20.76 3.84 C 20.28 3.24 19.68 3 19.2 3.12 C 18.72 3.24 16.92 3.6 14.4 6 C 14.4 6 17.52 9.12 19.2 12 C 20.88 14.88 21.48 17.04 20.88 20.04 C 20.88 20.04 20.64 18.48 18.96 16.2 C 17.76 14.52 14.04 10.32 12 8.4 C 9.96 10.32 6.24 14.52 5.04 16.2 C 3.36 18.48 3.12 20.04 3.12 20.04 Z M 3.12 20.04"
            fill="currentColor"
          ></path>
          <title>xbox</title>
        </svg>
      </button>
    </div>
  );
}
